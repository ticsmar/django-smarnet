...
from users.domain.exceptions.admin_exceptions import (
    GroupNotFoundError,
    UserNotFoundError,
)
...
def test_set_user_groups_unknown_group_raises() -> None:
    ...
    with (
        patch(
            "users.infrastructure.repositories.user_admin_repository_impl."
            "User.objects.prefetch_related",
            return_value=MagicMock(filter=MagicMock(return_value=queryset)),
        ),
        patch(
            "users.infrastructure.repositories.user_admin_repository_impl._resolve_groups",
