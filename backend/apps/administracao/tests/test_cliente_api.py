...
def auth_client(api_client: APIClient) -> APIClient:
    ...
    with (
        ...
        patch(
            "apps.administracao.presentation.views.cliente_views.resolve_actor_context",
            return_value=_ACTOR,
        ),
    ):
        yield api_client
...
@patch(
    "apps.administracao.presentation.views.cliente_views.build_list_clientes_use_case"
)
def test_list_clientes_api(mock_build: MagicMock, auth_client: APIClient) -> None:
    ...
    response = auth_client.get("/api/administracao/clientes/?search=ACME")
...
@patch("apps.administracao.presentation.views.cliente_views.build_get_cliente_use_case")
def test_get_cliente_api(mock_build: MagicMock, auth_client: APIClient) -> None:
    use_case = MagicMock()
    use_case.execute.return_value = _detail(codigo=7)
    mock_build.return_value = use_case

    response = auth_client.get("/api/administracao/clientes/7/")
...
def test_get_cliente_api_404(mock_build: MagicMock, auth_client: APIClient) -> None:
    ...
    response = auth_client.get("/api/administracao/clientes/99/")

    assert response.status_code == 404


@patch(
    "apps.administracao.presentation.views.cliente_views.build_grava_cliente_dados_gerais_use_case"
)
...
@patch(
    "apps.administracao.presentation.views.cliente_views.build_lookup_cliente_documento_use_case"
)
def test_lookup_documento_api(mock_build: MagicMock, auth_client: APIClient) -> None:
    ...
    response = auth_client.get(
        "/api/administracao/clientes/documento/?documento=12345678000199"
    )
...
@patch(
    "apps.administracao.presentation.views.cliente_views.build_consulta_cnpj_use_case"
)
def test_consulta_cnpj_api(mock_build: MagicMock, auth_client: APIClient) -> None:
    ...
    response = auth_client.post(
        "/api/administracao/clientes/cnpj/",
...
@patch(
    "apps.administracao.presentation.views.cliente_views.build_consulta_funcionario_use_case"
)
def test_consulta_funcionario_api(
    ...
    response = auth_client.post(
        "/api/administracao/clientes/funcionario/",
        {"cpf": "123.456.789-01"},
        format="json",
    )
...
@patch(
    "apps.administracao.presentation.views.cliente_views.build_create_cliente_from_funcionario_use_case"
)
def test_from_funcionario_api(mock_build: MagicMock, auth_client: APIClient) -> None:
    ...
    response = auth_client.post(
        "/api/administracao/clientes/from-funcionario/",
        {"cpf": "12345678901"},
        format="json",
    )
...
@patch(
    "apps.administracao.presentation.views.cliente_views.build_list_cliente_catalogs_use_case"
)
def test_catalogs_api(mock_build: MagicMock, auth_client: APIClient) -> None:
    ...
    paises = auth_client.get("/api/administracao/catalogos/paises/")
    estados = auth_client.get("/api/administracao/catalogos/estados/?pai_codigo=76")
    origens = auth_client.get("/api/administracao/catalogos/origens/")
...
def test_list_requires_auth(api_client: APIClient) -> None:
    response = api_client.get("/api/administracao/clientes/")
...
def test_create_requires_add_perm(mock_build: MagicMock, api_client: APIClient) -> None:
    ...
    django_user.has_perm.side_effect = lambda perm: (
        perm == "administracao_infrastructure.add_cliente"
    )
    use_case = MagicMock()
    use_case.execute.return_value = GravaClienteDadosGeraisOutputDTO(
