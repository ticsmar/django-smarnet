...
@patch(
    "apps.purchasing.presentation.views.recebimento_views.build_grava_fornecedor_use_case"
)
...
@patch(
    "apps.purchasing.presentation.views.recebimento_views.build_list_fornecedores_use_case"
)
def test_list_fornecedores_api(mock_build: MagicMock, auth_client: APIClient) -> None:
    ...
    response = auth_client.get("/api/purchasing/fornecedores/?search=ACME&ativo=1")
...
@patch(
    "apps.purchasing.presentation.views.recebimento_views.build_get_fornecedor_use_case"
)
def test_get_fornecedor_api(mock_build: MagicMock, auth_client: APIClient) -> None:
    ...
    response = auth_client.get("/api/purchasing/fornecedores/7/")
...
def test_get_fornecedor_api_404(mock_build: MagicMock, auth_client: APIClient) -> None:
    ...
    response = auth_client.get("/api/purchasing/fornecedores/99/")

    assert response.status_code == 404


@patch(
    "apps.purchasing.presentation.views.recebimento_views.build_ativa_fornecedor_use_case"
)
def test_ativa_fornecedor_api(mock_build: MagicMock, auth_client: APIClient) -> None:
    mock_build.return_value = MagicMock()
    response = auth_client.post("/api/purchasing/fornecedores/7/ativar/")
    assert response.status_code == 200
    assert response.data["ok"] is True


@patch(
    "apps.purchasing.presentation.views.recebimento_views.build_inativa_fornecedor_use_case"
)
def test_inativa_fornecedor_api(mock_build: MagicMock, auth_client: APIClient) -> None:
    mock_build.return_value = MagicMock()
    response = auth_client.post("/api/purchasing/fornecedores/7/inativar/")
    assert response.status_code == 200


@patch(
    "apps.purchasing.presentation.views.recebimento_views.build_grava_fornec_contato_use_case"
)
def test_grava_fornec_contato_api(
    ...
    response = auth_client.post(
        "/api/purchasing/fornecedor-contatos/",
        {
            "cod_fornec": 1,
            "nome": "Joao",
            "cargo": "Gerente",
...
@patch(
    "apps.purchasing.presentation.views.recebimento_views.build_list_fornec_contatos_use_case"
)
def test_list_fornec_contatos_api(
    ...
    response = auth_client.get("/api/purchasing/fornecedor-contatos/?for_codigo=1")
...
@patch(
    "apps.purchasing.presentation.views.recebimento_views.build_get_fornec_contato_use_case"
)
def test_get_fornec_contato_api(mock_build: MagicMock, auth_client: APIClient) -> None:
    ...
    response = auth_client.get("/api/purchasing/fornecedor-contatos/3/")
...
@patch(
    "apps.purchasing.presentation.views.recebimento_views.build_exclui_fornec_contato_use_case"
)
def test_exclui_fornec_contato_api(
    mock_build: MagicMock, auth_client: APIClient
) -> None:
    mock_build.return_value = MagicMock()
    response = auth_client.delete("/api/purchasing/fornecedor-contatos/3/")
    assert response.status_code == 200
    assert response.data["ok"] is True


@patch(
    "apps.purchasing.presentation.views.recebimento_views.build_list_msg_erros_use_case"
)
def test_list_msg_erros_api(mock_build: MagicMock, auth_client: APIClient) -> None:
    ...
    response = auth_client.get("/api/purchasing/msg-erros/?search=ORA")
...
def test_list_paises_api(mock_build: MagicMock, auth_client: APIClient) -> None:
    ...
    response = auth_client.get("/api/purchasing/paises/?search=Bras")
...
@patch("apps.purchasing.presentation.views.recebimento_views.build_get_pais_use_case")
def test_get_pais_api(mock_build: MagicMock, auth_client: APIClient) -> None:
    ...
    response = auth_client.get("/api/purchasing/paises/76/")
...
def test_get_pais_api_404(mock_build: MagicMock, auth_client: APIClient) -> None:
    ...
    response = auth_client.get("/api/purchasing/paises/999/")

    assert response.status_code == 404


def test_grava_fornecedor_requires_auth(api_client: APIClient) -> None:
    response = api_client.post("/api/purchasing/fornecedores/", {}, format="json")
    assert response.status_code == 401


def test_list_fornecedores_requires_auth(api_client: APIClient) -> None:
    response = api_client.get("/api/purchasing/fornecedores/")
    assert response.status_code == 401
...
def test_grava_fornec_contato_allowed_with_add_perm(
    ...
    django_user.has_perm.side_effect = lambda perm: (
        perm == "purchasing_infrastructure.add_forneccontato"
    )
    use_case = MagicMock()
    use_case.execute.return_value = GravaFornecContatoOutputDTO(
        cod_contato=5,
