...
O Smarnet 3.01 (PHP 5.2) segue em produção sobre o mesmo Oracle. Ao migrar telas, a gravação precisa de um critério: reusar a procedure do 3.01, replicar o DML em Python, ou criar objeto Oracle próprio do Django.

Reusar o package do 3.01 é o padrão quando ele serve. Parte dos objetos, porém, tem contrato incompatível com um caso de uso que também escreve no banco `default` do Django:
...
Senha: Smarnet Novo e 3.01 têm credenciais independentes — essa separação é uma das motivações da migração. O Django autentica `auth.User` com hash; `oracle_auth_repository_impl.py` apenas delega para `django_authenticate`.

## Decision

Três vias de escrita, nesta ordem de preferência:

1. **Tela migrada, contrato serve** — `callproc` na procedure do 3.01, no repositório. Vale inclusive com `COMMIT` interno, desde que a tela seja **um** save e a procedure propague erro.
...
- A via 2 duplica invariante que o 3.01 também aplica. Mitigação: DDL e package legados copiados em `docs/_scripts/` como referência de leitura, e a regra do 3.01 citada no repositório que a replica.
- Regras de alocação de chave que no 3.01 são `SELECT MAX(...) + 1` precisam de `select_for_update()` no lado Python, senão a corrida do legado é herdada.
